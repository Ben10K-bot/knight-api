...
const MUSLIM_JSON = 'https://raw.githubusercontent.com/itsSamBz/Islamic-Api/refs/heads/main/hadith/muslim.json';

router.get('/', async (req, res) => {
    res.json({
        status: true,
        creator: config.author,
        message: "Welcome to Muslim Hadith API. Use /all to get all hadiths, or /search with query parameters 'num' and/or 'page'."
    });
});

router.get('/all', async (req, res) => {
    try {
        const response = await fetch(MUSLIM_JSON);
        if (!response.ok) throw new Error(`Failed to fetch Muslim hadiths: ${response.status}`);
