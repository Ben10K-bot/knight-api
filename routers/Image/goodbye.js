...
router.get('/', async (req, res) => {
    const { background, icon, name } = req.query;
    if (!background || !icon || !name) {
        return res.status(400).json({
    status: false,
    creator: config.author || "dev.knight",
    message: "Missing parameters: background, icon, name"
  });
    }
    ...
    message: err.message
        ...
        ctx.fillText(`Goodbye ${decodeURIComponent(name)}`, 400, 280);
        
        ctx.font = '24px "Segoe UI"';
        ctx.fillText(`We'll miss you!`, 400, 320);
